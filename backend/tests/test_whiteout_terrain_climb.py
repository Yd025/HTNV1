...
class TerrainClimbTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_quad_climbs_vertically_in_home_frame_before_mission_goto(self):
        ...
        self.assertEqual(self.quad.calls[-1], ("goto", (71.9958, -94.8393, 234.), {}))
        ...
        self.quad.state.update(lat=71.996, alt=224., alt_msl=300.)
        self.adapter._air["quadcopter"].last_cmd_at = 0
        await self.adapter.send_command(goal)
        self.assertEqual(self.quad.calls[-1][1], (71.9958, -94.8393, 234.))
        self.quad.state.update(alt=234., alt_msl=310.)
        self.adapter._air["quadcopter"].last_cmd_at = 0
        await self.adapter.send_command(goal)
        self.assertEqual(self.quad.calls[-1], ("goto", (72., -94.8, 234.), {"yaw_deg": 30}))
    ...
    async def test_plane_climbs_over_known_water_before_crossing_the_map(self):
        ...
        await self.adapter.send_command(goal)
        self.assertEqual(self.plane.calls[-1], ("plane_goto", (*FORT_ROSS_CLIMB_LOITER, 310.)))
        self.assertFalse(any(call == ("plane_goto", (72., -94.8, 90.)) for call in self.plane.calls))
        self.plane.state.update(alt=310., alt_msl=310.)
        self.adapter._air["fixed-wing"].last_cmd_at = 0
        await self.adapter.send_command(goal)
        self.assertEqual(self.plane.calls[-1], ("plane_goto", (72., -94.8, 310.)))
    ...
    async def test_airborne_restart_climbs_locally_even_with_low_groundspeed(self):
        ...
        self.assertEqual(self.plane.calls[-1], ("plane_goto", (72.01, -94.81, 310.)))
    ...
    async def test_grounded_quad_takeoff_uses_site_floor(self):
        ...
        self.assertEqual(self.quad.calls[-1], ("takeoff", (234.,)))
    ...
    async def test_command_throttle_rechecks_current_clearance_arming_and_mode(self):
        ...
        for changed in ({"alt": 224., "alt_msl": 300.}, {"armed": False}, {"mode": "LAND"}):
            self.quad.state.update(alt=234., alt_msl=310., armed=True, mode="GUIDED")
