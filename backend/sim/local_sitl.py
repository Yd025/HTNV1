...
class KinematicCraft:
    ...
    def step(self, dt: float) -> None:
        ...
        if self.cmd.lat is None or self.cmd.lon is None:
            return
        tn, te = ll_to_ne(self.cmd.lat, self.cmd.lon)
        ...
        self.north, self.east = clamp_arena(self.north, self.east)
        self.heading = wrap_heading(bearing_deg(*ne_to_ll(self.north - dn, self.east - de), *ne_to_ll(self.north, self.east)))
    ...
    def state(self, mavlink: bool) -> VehicleState:
        lat, lon = ne_to_ll(self.north, self.east)
        return VehicleState(
...
class LocalSitlAdapter:
    ...
    def __init__(self) -> None:
        ...
        self._kin: dict[str, KinematicCraft] = {s["vehicle_id"]: KinematicCraft(s) for s in _fleet_spec()}
        self._bridges: dict = {}
        self._target = TargetSim()
        ...
        for spec in _fleet_spec():
            ...
            if mav_ok:
                ...
                if snap.get("lat") is not None:
                    kin.north, kin.east = ll_to_ne(float(snap["lat"]), float(snap["lon"]))
