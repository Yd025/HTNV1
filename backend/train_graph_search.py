...
def source_hashes():
    ...
    return {name:hashlib.sha256((Path(__file__).parent/name).read_bytes().replace(b"\r\n",b"\n")).hexdigest()
            for name in ("graph_search.py","train_graph_search.py","flight_policy.py","plane_shadow.py")}
...
def train(args):
    ...
    report={"schemaVersion":1,"mode":"synthetic-terrain-graph","missionVersion":algorithm,"algorithm":algorithm,
            ...
            "limitations":["Offline synthetic experiment on source-derived WORLD XY terrain; not live ArcticSim observations or official challenge scores.",
              ...
              "Tower bases remain fixed throughout each mission. Tower cameras scan 6 deg/s until a candidate observation, then aim at the observation-derived estimate within source pitch limits; tower slew dynamics are simplified. Quad camera is rigidly body-mounted at source-derived -20 degrees with no independent gimbal aim; the quad pursues a viewing standoff derived from modeled sea height and mount depression. Plane camera remains fixed near -8 degrees. Coordinated support uses moving offset approaches, turns before the camera blind zone and outward repositioning; fixed-wing camera gaps during these turns are expected and require quad/tower support, not invented receiver observations.",
