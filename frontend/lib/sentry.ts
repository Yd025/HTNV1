const slug = (value: string | undefined) => value && /^[a-z0-9][a-z0-9-]*$/.test(value.trim()) ? value.trim() : undefined;
export const SENTRY_ORG = slug(process.env.NEXT_PUBLIC_SENTRY_ORG);
export const SENTRY_PROJECT = slug(process.env.NEXT_PUBLIC_SENTRY_PROJECT);
export const SENTRY_BASE = SENTRY_ORG ? `https://${SENTRY_ORG}.sentry.io` : "https://sentry.io";
