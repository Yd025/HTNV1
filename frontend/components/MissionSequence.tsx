...
const phases: Record<string, { title: string; step: number; detail: string }> = {
  coordinated_search: { title: "Fleet searching together", step: 1, detail: "The plane sweeps wider water, the quad checks nearby gaps, and towers scan fixed sectors. Any sensor can initiate a contact." },
  search: { title: "Fleet searching together", step: 1, detail: "Aircraft patrol complementary sectors while the towers scan. Routes use sensor observations, never the hidden boat position." },
  contact_confirm: { title: "Verifying a sensor sighting", step: 1, detail: "A second fresh, consistent observation is needed to confirm the contact." },
  ...
  lost: { title: "Contact lost", step: 1, detail: "The estimate expired. Sensors continue searching for fresh evidence." },
};
...
export default function MissionSequence({ frame, pending = false }: { frame?: GraphFrame; pending?: boolean }) {
  ...
  return <section className={styles.missionSequence} aria-label="Coordinated mission sequence">
    <ol>{["Set strategy", "Search & confirm", "Coordinate aircraft", "Maintain track"].map((label, index) => <li key={label} aria-current={index === state.step ? "step" : undefined} data-complete={index < state.step}><span>{index + 1}</span>{label}</li>)}</ol>
