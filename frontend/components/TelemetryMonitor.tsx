...
const backendStatusNotices = {
  warming: {
    label: "Warming up",
    title: "Simulation warming up.",
    description: "The backend reports that it is preparing mission telemetry.",
  },
  failed: {
    label: "Backend failure",
    title: "The simulation reported a failure.",
    description: "The last supplied state remains available. Check the simulation service for more detail.",
  },
  complete: {
    label: "Run complete",
    title: "The simulation run is complete.",
    description: "The backend reported completion. Its final supplied state remains available for inspection.",
  },
} as const;

/** A reported run outcome is distinct from socket state and frame age. */
export function getBackendStatusNotice(status: SwarmState["status"]) {
  return status === "warming" || status === "failed" || status === "complete"
    ? backendStatusNotices[status]
    : null;
}
...
export default function TelemetryMonitor({ telemetry }: { telemetry: MissionTelemetry }) {
  ...
  const readiness = backendNotice?.label ?? (isFresh ? "State available" : hasReceived ? "Stale state retained" : "Awaiting state");
    ...
    <section className="telemetry-monitor" aria-labelledby="telemetry-monitor-title">
      <div className="monitor-heading">
        ...
        <span className={`monitor-state ${isFresh ? "is-fresh" : ""}`} role="status">
          ...
          {backendNotice?.label ?? (isFresh ? "Receiving" : hasReceived ? "Stale" : connection === "live" ? "Waiting for telemetry" : transport)}
        </span>
      </div>
      ...
      {backendNotice && <p className="monitor-note">{backendNotice.description}</p>}
      {!backendNotice && connection === "live" && !isFresh && (
