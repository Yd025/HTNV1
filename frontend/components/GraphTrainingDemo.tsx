...
const pct = (value: number | undefined) => Number.isFinite(value) ? `${value!.toFixed(1)}%` : "—";
...
function MetricTable({ report }: { report: GraphReport }) {
  const rows: { label: string; value: (m: GraphMetrics) => string; title: string }[] = [
    { label: "Boats detected", value: m => pct(m.detectionRate), title: "Percentage of all test missions with at least one modeled detection." },
    ...
    { label: "Tracking custody", value: m => pct(m.custodyPct), title: `Fraction of the full mission with an observation no older than ${report.protocol.freshnessS} seconds.` },
    ...
    { label: "Reporting-source changes", value: m => num(m.handoffs), title: "Mean changes of nearest reporting sensor, including reacquisition after gaps. Not coordinated handoff success." },
  ];
  return <div className={styles.tableWrap}><table><caption>The same {report.protocol.testEpisodes} unseen missions · {report.protocol.horizonS} seconds each</caption><thead><tr><th scope="col">Measure</th><th scope="col">Sweep baseline</th><th scope="col">Untrained graph</th><th scope="col">Trained model</th></tr></thead><tbody>{rows.map(row => <tr key={row.label}><th scope="row" title={row.title}>{row.label}</th><td>{row.value(report.metrics.baseline)}</td><td>{row.value(report.metrics.untrained)}</td><td>{row.value(report.metrics.trained)}</td></tr>)}</tbody></table></div>;
}
...
export default function GraphTrainingDemo() {
  ...
  if (!report || !profile || !frame) return <section className={styles.demo} aria-label="Arctic search learning"><div className={styles.heading}><h2>Teach the fleet to search</h2><p role={error ? "alert" : "status"}>{error ?? "Loading the terrain, trained model, and test missions…"}</p></div>{error && <button className={styles.reload} onClick={() => window.location.reload()}>Reload experiment</button>}</section>;
  ...
  return <section className={styles.demo} aria-labelledby={`${unique}-title`}>
    ...
    {!training && <div className={styles.workbench}>
      <div className={styles.stage}>
        ...
        <svg ref={mapRef} className={styles.map} viewBox="0 0 770 605" role="img" aria-label="Fort Ross terrain map with two towers, a quadcopter, a fixed-wing drone, and a boat. Use placement controls to edit positions." onPointerDown={mapClick}>
          ...
          <g clipPath={`url(#${unique}-map)`}>
            ...
            {[...towers, ...displayDrones].map((source, i) => {
              ...
              const heading = source.id.includes("tower") ? (!custom ? visualFrame!.towerHeadings?.[i] : undefined) ?? source.heading : source.heading;
              const visible = !custom && frame.sources.includes(source.id);
              return <g key={`${source.id}-view`} style={{ color: color(source.id) }}>
                ...
                {showFootprints && sensor && <polygon points={points(cameraFootprint({ ...source, heading }, sensor))} className={styles.cone} />}
            ...
            {distanceGuide && <g className={link ? styles.sightLine : styles.guideLine}><line x1={project(distanceGuide.source).x} y1={project(distanceGuide.source).y} x2={boat.x} y2={boat.y} /><text x={(project(distanceGuide.source).x + boat.x) / 2} y={(project(distanceGuide.source).y + boat.y) / 2 - 9}>{Math.round(distanceGuide.distanceM)} m</text></g>}
        ...
        <div className={styles.mapLegend}><span><i className={styles.solidLegend} />Nearest reporting sensor · {frame.t} s sample</span><span><i className={styles.dashLegend} />Distance guide when unseen</span><span><i className={styles.routeLegend} />Planned drone route</span></div>
        <p className={styles.mapNote}>Sweeps show scan direction; the 1,500 m rings are a distance reference. Detection uses the full camera view and terrain. Turn on exact footprints to inspect the camera geometry. Boat truth is shown for evaluation only.</p>
      ...
      <aside className={styles.controls} aria-label="Experiment controls">
        <div><h3>Test a placement</h3><p>Place the towers on land and the boat on water. The drones choose their routes from what the sensors observe.</p><div className={styles.tools}>{([['inspect', 'Inspect'], ['boat', 'Place boat'], ['tower0', 'Move tower 1'], ['tower1', 'Move tower 2']] as const).map(([value, label]) => <button key={value} disabled={busy} aria-pressed={mode === value} onClick={() => setMode(value)}>{label}</button>)}</div>
    ...
    <div className={styles.evidence}>
      ...
      <div className={styles.resultHeading}><h3>What happened on unseen boats</h3><p><strong>{pct(trainedRate)}</strong> found · {difference >= 0 ? "+" : ""}{num(difference)} percentage points versus the sweep baseline. Best tested configuration; individual missions can regress.</p></div>
      ...
      <div className={styles.trainingChart}><Plot label="Completed benchmark: unseen boats found over time" rows={report.detectionCurve.map(value => ({ x: value.t, values: [value.baseline, value.untrained, value.trained] }))} series={[{ label: "Sweep", color: "var(--text-muted)", dash: "4 4" }, { label: "Untrained graph", color: "var(--warning)", dash: "8 3" }, { label: "Trained", color: "var(--status)" }]} xLabel="Mission seconds · misses stay in denominator" maxX={report.protocol.horizonS} /></div>
      <MetricTable report={report} />
      {report.comparison && <p className={styles.metricNote}>Mean capped delay improved by {num(report.comparison.meanSecondsSaved, " s")} versus the sweep; paired 95% bootstrap interval {num(report.comparison.pairedBootstrap95S[0])}–{num(report.comparison.pairedBootstrap95S[1], " s")}. Training can trade coverage or position accuracy for faster detection; compare each row.</p>}
      {report.metrics.trained.bySource && <div className={styles.contributions}><h4>Who contributed observations</h4><p>Share of positive sensor samples across the {report.protocol.testEpisodes} trained-policy test missions. More than one sensor can report the same boat.</p>{Object.entries(report.metrics.trained.bySource).map(([id, count]) => { const total = Object.values(report.metrics.trained.bySource!).reduce((sum, value) => sum + value, 0); const share = total ? count / total * 100 : 0; return <div key={id}><span>{assetLabel(id)}</span><div className={styles.shareTrack}><i style={{ width: `${share}%`, background: color(id) }} /></div><strong>{pct(share)}</strong><small>{count.toLocaleString()} samples</small></div>; })}</div>}
      <p className={styles.metricNote}>These are local evaluation measures covering the slide’s scoring categories, not Dominion’s official score formula. Accuracy uses synthetic noisy positions. Search decisions and reporting-sensor selection run automatically; source changes include reacquisition after gaps.</p>
      </details>
      ...
      <details className={styles.details}><summary>Model assumptions and research</summary><p>The compact model learns boat transitions and search priorities. A* finds a shortest route on the chosen graph to a selected search point; it does not prove the fastest unknown-target search or a globally optimal tower pair.</p><ul>{report.limitations.map((item, i) => <li key={i}>{item}</li>)}</ul><p>Based on <a href="https://ai.stanford.edu/~nilsson/OnlinePubs-Nils/PublishedPapers/astar.pdf" target="_blank" rel="noreferrer">Hart, Nilsson & Raphael’s A*</a>, <a href="https://arxiv.org/abs/1902.10182" target="_blank" rel="noreferrer">obstacle-aware informative search</a>, and <a href="https://publications.ri.cmu.edu/sensor-planning-for-large-numbers-of-robots" target="_blank" rel="noreferrer">cooperative sensor planning</a>. The supplied ArcticSim slides define the four assets and scoring categories. Installed source provides current camera dimensions and terrain.</p><p>Local experiments do not reposition the live fleet. The live mission and camera feeds remain below.</p></details>
    </div>
  </section>;
}
