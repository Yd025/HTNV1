...
export default function TickPerformance({ summary, isFresh }: { summary: TickSummary; isFresh: boolean }) {
  ...
  const stageScale = Math.max(0.01, ...view.stages.map(stage => stage.p95));
  ...
  return <section className={s.card} aria-labelledby="stages-title" data-testid="tick-performance">
    ...
    {view.count ? <>
      ...
      <figure className={s.stages} aria-label="95th percentile duration by controller stage">
        <figcaption className={s.caption}>Where the controller spends time · stage p95 · bars scaled to the slowest stage</figcaption>
        {view.stages.map(stage => <div className={s.stage} key={stage.op}><div><strong>{stage.op}</strong>{stage.errors > 0 && <span>{stage.errors} failed spans</span>}</div><div className={s.bar} aria-hidden="true"><i style={{ width: `${stage.p95 / stageScale * 100}%`, minWidth: stage.p95 > 0 ? 2 : 0 }} /></div><span>{ms(stage.p95)}</span></div>)}
      </figure>
      <details className={s.guideDetails}><summary>Detailed stage measurements</summary>
