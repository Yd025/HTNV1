...
export default function SentryPanel({ telemetry, tickSummary }: { telemetry: MissionTelemetry; tickSummary: TickSummary }) {
  ...
  return <div className={s.panel}>
    <section className={s.hero} aria-labelledby="sentry-title">
      <div><span className={s.eyebrow}>Observability{SENTRY_PROJECT ? ` / ${SENTRY_PROJECT}` : ""}</span><h2 id="sentry-title">See what shaped the mission.</h2><p>Follow a slow tick, inspect a broken flow, and connect each finding to the run that produced it.</p></div>
      <div className={s.heroActions}><a className={s.primary} href={sentryLink("issues")} target="_blank" rel="noreferrer">Open Sentry <span aria-hidden="true">↗</span></a><span>{SENTRY_ORG && SENTRY_PROJECT ? `${SENTRY_ORG} / ${SENTRY_PROJECT}` : "Choose your project in Sentry"}</span></div>
    </section>
