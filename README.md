...
In the Sentry tab, **Open ship camera** embeds the native simulator with **Follow ship** enabled. It follows ArcticSim's `target_vessel` world object before each native render, preserving orbit and zoom. Disconnects, missing objects and invalid positions pause following; **Stop following ship** returns free camera control. This observer view uses simulator truth only to position the viewing camera; it never feeds the detector, estimator, or vehicle control loop. It is separate from the backend's local synthetic run when `ADAPTER=local`.
