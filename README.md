...
The current coordinated run (seed 20260920) compared all three strategies on the same 200 unseen missions: tower-first/default sites detected **24.5%**, default coordinated flights/default sites **46.0%**, and trained flights/selected sites **68.0%**. Aircraft tracking custody increased from **9.1% → 19.0% → 24.2%**; mean longest contact gap decreased from **230.2 → 173.6 → 114.8 seconds**. The trained strategy saved 85.6 seconds of capped detection delay against tower-first (paired 95% bootstrap interval 71.31–100.05 seconds), but increased mean aircraft travel from **4.73 to 7.10 km**. These are combined tower-and-flight-policy gains under the synthetic sensor model, not isolated flight-only gains or proof of real-camera performance. The frozen [model](frontend/public/experiments/surveillance-model.json) and [complete results](frontend/public/experiments/surveillance-report.json) retain all misses and source fingerprints.

Train coordinated surveillance from `backend/` without replacing the historical artifacts:

```powershell
python -m pip install -r requirements-training.txt
python -B train_graph_search.py --algorithm coordinated-surveillance-v1 --seed 20260920 --output ../frontend/public/experiments/surveillance-report.json --model-output ../frontend/public/experiments/surveillance-model.json --progress ../frontend/public/experiments/surveillance-progress.json
...
| `lookaheadS` | 15 s | 0–40 s | Predicted continuation for aircraft/tower support; quad follow caps lead at 8 s. |
| `supportOffsetM` | 350 m | 100–1000 m | Lateral separation of fixed-wing observation passes. |
