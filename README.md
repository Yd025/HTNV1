...
Open the [standalone HTML training report](frontend/public/experiments/training-report.html) locally, or visit `/experiments/training-report.html` on the frontend. It includes the terrain map, selected coordinates, algorithm explanation, condition filters, detection curves and every held-out mission. The full run took **268.23 seconds (4 minutes 28 seconds)** including training, validation and testing; isolated training time was not recorded. Companion [JSON evidence](frontend/public/experiments/training-report-data.json) and [600-row CSV](frontend/public/experiments/training-missions.csv) preserve the measurements, definitions and provenance. The original model, report with replays and terrain profile remain alongside them. Rebuild the artifact without retraining using `node frontend/scripts/build-training-report.cjs` from the repository root.
