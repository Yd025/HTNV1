...
**Saturday** — `ADAPTER=whiteout` against arctic-sim `udpout`. Freeze BT gains unless their metric definition differs. Camera detector lives in `sim/detector.py`. `POST :8010/api/tracks` is still open. Swap HUD terrain only if they publish a real mesh.
