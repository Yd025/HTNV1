...
**September 20 update:** The user authorized coordinated surveillance in both simulation and the Cant Catch Me game, with a named algorithm and learned aircraft flight policies alongside tower placement. `coordinated-surveillance-v1` supersedes tower-only launch gating when selected: plane and quad actively search complementary sectors, any sensor can initiate a repeatedly confirmed contact, and aircraft support tracking/reacquisition from observations. Keep `tower-first-v2` as the historical comparison and preserve its saved recordings. Six bounded flight settings are selected on training/validation data before untouched tests. Game rules, policy and tower layout are pinned per attempt; legacy recordings retain their original behavior. Game fairness gates remain. A synthetic flight-policy result is not image-model training, camera validation, or a verified physical flight plan. All runtime commands still go through the adapter.

Historical tower-first behavior remains available: optimize and freeze two tower sites, require repeated fresh tower evidence to dispatch aircraft, confirm handoff only from fresh accepted aircraft detections, and coast/reacquire/expire lost contacts. No truth coordinates enter decision logic for either algorithm. Optional image models stay separate from the fast loop and fail visibly when unavailable. Physical sensor calibration and image training require data.
