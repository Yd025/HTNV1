...
if ($site.name -ne 'fort_ross') {
    throw 'This WHITEOUT adapter is calibrated for Fort Ross. Select fort_ross in ArcticSim before launching.'
}
...
if ($Check) {
    [pscustomobject]@{ Simulator = $site.name; BackendRunning = [bool]$health; DashboardRunning = [bool]$dashboard; GameRunning = [bool]$game }
    return
}
