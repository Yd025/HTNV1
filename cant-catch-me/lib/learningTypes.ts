import type { FLIGHT_ALGORITHM, FlightPolicy, SearchPoint, SurveillanceRole } from './surveillance';
...
export type Layout = { version: number; towers: TowerPosition[]; createdAt: string; reason: string; algorithm?: typeof FLIGHT_ALGORITHM; flightPolicy?: FlightPolicy };
...
export type LiveFrame = {
  ...
  drones: { id: string; x: number; z: number; heading: number; detecting: boolean; tagProgress: number; role?: SurveillanceRole; target?: SearchPoint }[];
  plane: { x: number; z: number; heading: number; detecting: boolean; role?: SurveillanceRole; target?: SearchPoint };
...
export type ReplayScore = {
  ...
  surveillance?: { detectionRate: number; meanFirstDetectionSeconds: number | null;
    visualContactFraction: number; longestContactGapSeconds: number; meanAircraftDistanceM: number;
    planeVisualContactFraction?: number };
};
